...
export default function AssetFingerprint() {
    ...
    const callback = (key: string) => {
        ...
        if (key === "用户信息") {
            ...
            setUserInfo({ inputList: [{ placeholder: "服务器IP/端口/进程名", label: "搜 索：", name: "keyword" }], searchCondition: searchCondition, searchReset: searchReset })
            ...
            setUserInfo({ inputList: [{ placeholder: "服务器IP/进程名/进程ID/路径/文件md5", label: "进程搜索：", name: "keyword" }], searchCondition: searchCondition, searchReset: searchReset })
            ...
            setUserInfo({ inputList: [{ placeholder: "服务器IP/数据库名称/类型/端口/安装路径", label: "搜 索：", name: "keyword" }], searchCondition: searchCondition, searchReset: searchReset })
            ...
            <Tabs defaultActiveKey="1" onChange={callback} className="detail">
                ...
                <TabPane tab=" web站点" key="web站点">
                    ...
                    <div style={{ marginTop: "20px" }}>
                        <span>服务类型：</span>
                        <Radio.Group defaultValue={0} buttonStyle="solid" onChange={onChangeRadio}>
                            <Radio.Button value={0}>全部<span>({100})</span></Radio.Button>
                            <Radio.Button value={1}>nginx<span>({100})</span></Radio.Button>
                            <Radio.Button value={2}>apache<span>({100})</span></Radio.Button>
                            <Radio.Button value={3}>IIS<span>({100})</span></Radio.Button>
                        </Radio.Group>
                    </div>
                    <TableOptional props={optionalTransferInfo} />
                </TabPane>
